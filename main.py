...
def train(model, device, train_loader, optimizer, criterion, epoch):
    ...
    for batch_idx, (data, target) in tqdm(enumerate(train_loader)):
        data = data.to(device)
        target = target.to(device) ##data [48, 3, 256, 256] target ([48, 1])
        output = model(data)
    ...
    logger.info('Train Epoch: {} Train average loss: {:.4f}, Train accuracy: {}/{} ({:.0f}%)'.format(
        epoch, train_loss_avg, correct, total, accuracy))

    # 在Tensorboard中记录训练损失和准确率
    writer.add_scalars('Train Metrics', {'Train average loss': train_loss_avg,
                                         'Train accuracy': accuracy}, batch_idx)
    # 在Tensorboard中记录训练损失和准确率
    writer.add_scalars('Train Metrics', {'Train average loss': train_loss_avg,
                                         'Train accuracy': accuracy}, epoch)
    writer.flush()
    save_path = os.path.join(args.out_dir, args.model + f'_model_{epoch}.pth')
    torch.save(model.state_dict(), save_path)
    logger.info(f'Saved model to {save_path}')

    return train_loss_avg, accuracy
...
def validate(model, device, val_loader, criterion, epoch):
    ...
    logger.info('Validation set: Val average loss: {:.4f}, Val accuracy: {}/{} ({:.0f}%), Val precision: {:.4f}, Val recall: {:.4f}, Val f1: {:.4f}'.format(
        val_loss_avg, correct, total, accuracy, precision, recall, f1))
    writer.add_scalars('Validation Metrics', {'Val average loss': val_loss_avg,
                                              'Val accuracy': accuracy,
                                              'Val precision': precision,
                                              'Val recall': recall,
                                              'Val f1': f1}, epoch)
    writer.flush()
    return val_loss_avg, accuracy, precision, recall, f1
...
def main():
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        val_loss, val_acc, precision, recall, f1 = validate(model, device, val_loader, criterion, epoch)
        logger.info('Epoch: {}, Train Loss: {:.4f}, Train Acc: {:.2f}%, Val Loss: {:.4f}, Val Acc: {:.2f}%, Val Precision: {:.2f}%, Val Recall: {:.2f}%, Val F1: {:.2f}%'.format(
